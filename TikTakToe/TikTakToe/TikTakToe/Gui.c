...
void HandleButtonPress(HWND hwnd, WPARAM wParam) {
    ...
    if (hButton && GetWindowTextLength(hButton) <= 0) {
        int ConfirmChoice = PlayerChoice(buttonID);
        if (!ConfirmChoice) return;

        int botPosition, winner;
        ...
        if (winner) {
            ...
            } else if (winner == 2) {  // Bot wins
                MessageBox(hwnd, "Bot wins!", "Game Over", MB_OK);
            }
            return;  // Exit function, preventing bot's move if player won
        }

        botPosition = BotChoice();

        if (botPosition) {
            HWND hBotButton = GetDlgItem(hwnd, botPosition + 1);
            SetWindowText(hBotButton, "O");
            
            // Check if bot won after its move
            winner = CheckWinner();
